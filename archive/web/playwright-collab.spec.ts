...
test('collaborative editor syncs between two clients', async ({ browser }) => {
  ...
  await pageA.addInitScript(() =>
    localStorage.setItem('userName', 'playwright-A')
  );
  await pageB.addInitScript(() =>
    localStorage.setItem('userName', 'playwright-B')
  );
  ...
  const beforeModels = await pageA.evaluate(() => {
    ...
    return {
      count: models.length,
      values: models.map((md: any) => md.getValue()),
    };
  });
  ...
  try {
    await pageA.waitForEvent('console', {
      predicate: m =>
        m.text().toLowerCase().includes('websocket status') ||
  ...
  const yTextB = await pageB.evaluate(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (globalThis as any).__udp_test
