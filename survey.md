...
We are pivoting UDP from an IDE-heavy direction to a cross-platform workflow tool (CLI + sync + mobile + extension). This survey validates whether the new direction is worth proceeding with. The survey takes ~3 minutes.
...
If UDP shipped this workflow tool in the next few sprints, would you actively try it?
- Yes
- No
