...
const MockNativeMethods = jest.requireActual(path.join(rnDir, 'jest/MockNativeMethods'));
const mockComponent = jest.requireActual(path.join(rnDir, 'jest/mockComponent'));
...
Object.defineProperties(global, {
  ...
  performance: { configurable: true, enumerable: true, value: { now: jest.fn(Date.now) }, writable: true },
  requestAnimationFrame: { configurable: true, enumerable: true, value: cb => setTimeout(() => cb(jest.now()), 0), writable: true },
...
jest
  ...
  .mock(path.join(rnDir, 'Libraries/BatchedBridge/NativeModules'), () => ({
    ...
    PlatformConstants: { getConstants() { return {}; } },
  ...
  .mock(path.join(rnDir, 'Libraries/Components/View/View'), () =>
    mockComponent(path.join(rnDir, 'Libraries/Components/View/View'), MockNativeMethods),
  )
  .mock(path.join(rnDir, 'Libraries/Components/View/ViewNativeComponent'), () => {
  ...
  .mock(path.join(rnDir, 'Libraries/Components/TextInput/TextInput'), () =>
    mockComponent(path.join(rnDir, 'Libraries/Components/TextInput/TextInput'), {
      ...MockNativeMethods,
      isFocused: jest.fn(),
      clear: jest.fn(),
  ...
  .mock(path.join(rnDir, 'Libraries/Components/ActivityIndicator/ActivityIndicator'), () => ({
    __esModule: true,
    default: mockComponent(path.join(rnDir, 'Libraries/Components/ActivityIndicator/ActivityIndicator'), null, true),
  }))
  .mock(path.join(rnDir, 'Libraries/NativeComponent/NativeComponentRegistry'), () => ({
    get: jest.fn((name) => jest.requireActual(path.join(rnDir, 'jest/mockNativeComponent')).default(name)),
    getWithFallback_DEPRECATED: jest.fn((name) => jest.requireActual(path.join(rnDir, 'jest/mockNativeComponent')).default(name)),
  ...
  .mock(path.join(rnDir, 'Libraries/Utilities/verifyComponentAttributeEquivalence'), () => function () {})
  .mock(path.join(rnDir, 'Libraries/ReactNative/RendererProxy'), () =>
    jest.requireActual(path.join(rnDir, 'Libraries/ReactNative/RendererImplementation')),
  );
