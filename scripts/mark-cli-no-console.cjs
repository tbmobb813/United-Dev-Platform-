...
const patterns = ['scripts/**/*.{js,ts,cjs,mjs,jsx,tsx}', 'bin/**/*.{js,ts,cjs,mjs}'];
    ...
    function visit(node) {
      if (ts.isCallExpression(node)) {
        ...
        if (ts.isPropertyAccessExpression(expr)) {
          ...
          if (obj === 'console') { found = true; return; }
        }
      }
...
const files = patterns.flatMap(p => glob.sync(p, { cwd: repoRoot, nodir: true, ignore })).map(p => path.join(repoRoot, p));
const edits = [];
...
for (const filePath of files) {
  ...
  const isCli = src.startsWith('#!') || /(^|\/)scripts\//.test(filePath) || /(^|\/)bin\//.test(filePath);
  if (!isCli) continue;
  ...
  if (src.startsWith('#!')) {
    ...
    if (firstLineEnd === -1) {
      ...
      out = src.slice(0, firstLineEnd + 1) + '/* eslint-disable no-console */\n' + src.slice(firstLineEnd + 1);
    }
  } else {
