  ...
  program
    ...
    .action(async options => {
      ...
      const registerRes = await fetch(
        `http://localhost:${port}/api/devices/register`,
        {
          method: 'POST',
      ...
      await new Promise(resolve => {
        rl.question(
          chalk.yellow('Confirm device pairing? (y/n): '),
          async answer => {
            if (answer.trim().toLowerCase() === 'y') {
              ...
              if (!authToken) {
                rl.question(
                  chalk.cyan('Enter pairing auth token: '),
                  tokenInput => {
                    rl.close();
              ...
              async function confirmDevice(authToken: string) {
                const confirmRes = await fetch(
                  `http://localhost:${port}/api/devices/confirm`,
                  {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (confirmRes.ok) {
                  ...
                  try {
                    ...
                    const { ProjectSyncManager } = await import(
                      '@udp/editor-core'
                    );
                    ...
                    ws.on('open', () => {
                      ...
                      logger.info(
                        chalk.green('Monitoring project for changes...')
                      );
                    });
                    ...
                    logger.warn(
                      { err: error },
                      'File sync setup failed, continuing with pairing only'
                    );
                  }
                } else {
